...
from bsoid_utils import compute

def load_data(path, output_folder, feature_processing, create_csv = True):
    '''
        ...
        if os.path.isdir(subfolder_path):
            ...
            for filename in os.listdir(subfolder_path):
                if filename.endswith('.csv'):
                    ...
                    if  feature_processing == "subtract_centroid":
                        ...
                        for start in range(0, n_samples, frames):
                            ...
                            datapoints_reshaped = chunk.reshape((frames, num_keypoints * num_feats))
                            all_datapoints_reshaped.append(datapoints_reshaped)
                            video_names.append(video_name)
                            labels.append(subfolder)  # Append the name of the subfolder to labels
                            chunk_count += 1
                        
                        #print(f"datapoints_reshaped shape: {np.shape(datapoints_reshaped)}")
                        
                        # print(f"Number of chunks: {chunk_count}")
                        chunks_per_video.extend([chunk_count] * chunk_count)
                    
                    elif feature_processing == "bsoid":
                        # print("x: ", x)
                        # print("y: ", y)
                        ...
                        for start in range(0, n_samples, frames):
                            ...
                            chunk = keypoints[start:end]
                            datapoints_reshaped = chunk.reshape((frames, num_keypoints * num_feats))
                            datapoints_reshaped_reshape = datapoints_reshaped.reshape((1, datapoints_reshaped.shape[0], datapoints_reshaped.shape[1]))

                            scaled_features, features = compute(datapoints_reshaped_reshape, framerate=30)
                            scaled_features_tr = np.transpose(scaled_features)

                            # print(f"Shape of input features: {np.shape(scaled_features_tr)}")
                            
                            all_datapoints_reshaped.append(scaled_features_tr)
                            video_names.append(video_name)
                            labels.append(subfolder)  # Append the name of the subfolder to labels
                            chunk_count += 1
    ...
    all_datapoints_reshaped = np.array(all_datapoints_reshaped)
    
    print(f"All keypoints reshaped shape: {all_datapoints_reshaped.shape}")
    #print("All keypoints reshaped:")
    #print(all_datapoints_reshaped)
    ...
    return all_datapoints_reshaped
